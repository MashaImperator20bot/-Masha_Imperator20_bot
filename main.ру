...
if __name__ == "__main__":
    ...
    model_thread.start()
    print("Бот запущен...")
    bot.infinity_polling() Привет, создай бота для телеграм который по команде /бан удаляет сообщения пользователя который написал сообщение до этой команды бесконечно, и по команде /экстерминатус удаляет пользователи который написал сообщение для этой команды.еще если написать разбан то это 1 раз сработает , потом что бы снять бан нужно будет написать сообщение со словом рыбалка или смайликом но об этом нигде не упоминается, также пусть в боте по команде /праймус будет нейросеть, по команде /антипраймус бот по умолчанию на каждое 11 сообщение отвечает либо гав либо ГАВ либо (довольный) гав, либо Ррррр, либо гав?, либо гав..., это его привычный режим который стоит когда нет команд. По команде /юристус. Он начинает защищать пользователя который ее написал перед этим он анализирует сообщения. Чтобы узнать ситуацию.
